...
- **Automation**: domain events go to a transactional outbox and are swept out over an HMAC-signed webhook to a **Cloudflare Worker** (`automation/` + `cloudflare-worker/`), which owns the integration with Telegram; this app owns none of it
...
- **Automation**: Cloudflare Workers (free tier, 100k requests/day). No container, so nothing to keep awake and no cold start - see the free-tier note under Architecture Decisions
...
### Tự động hóa
- ✅ Sự kiện nghiệp vụ → outbox có retry/backoff → webhook ký HMAC-SHA256 → Cloudflare Worker → Telegram
- ✅ Chạy 24/7, không ngủ, không cold start: chốt hóa đơn ở POS → Telegram kêu trong ~15 giây, bất kể giờ nào
- ✅ Worker nằm trong repo (`cloudflare-worker/`), test bằng `node cloudflare-worker/verify-worker.mjs` — nạp thẳng file sẽ deploy, và khoá lại đúng vector HMAC mà test Java ghim
- ✅ Dispatcher phân biệt "đích đang khởi động" với "cấu hình sai": lỗi mạng không tiêu lượt thử trong 30 phút đầu, 404 thì có
...
│
├── cloudflare-worker/
│   ├── src/index.js      # Verifies the HMAC, formats the message, calls Telegram
│   ├── verify-worker.mjs # Runs that exact file under Node, no Cloudflare needed
│   ├── send-test-event.mjs # Fires one signed event at the deployed Worker
│   └── README.md         # Deploy steps, signature contract, troubleshooting
│
