...
import com.ut.edu.backend.review.ReviewRepository;
 ...
 * and delegate the actual model call to DeepSeekService
 */
@Service
@Primary
@Slf4j
public class AIService {
    ...
    private final com.ut.edu.backend.order.OrderRepository orderRepository;
    private final com.ut.edu.backend.review.ReviewRepository reviewRepository;
    private final DeepSeekService deepSeekService;

    public AIService(ProductRepository productRepository,
                                UserRepository userRepository,
                                RestTemplate restTemplate,
                                com.ut.edu.backend.order.OrderRepository orderRepository,
                                com.ut.edu.backend.review.ReviewRepository reviewRepository,
    ...
    public AIAnalysisResponse analyzeRevenuePerformance(String period) {
        try {
            ...
            List<com.ut.edu.backend.order.OrderStatus> revenueStatuses = List.of(
                    com.ut.edu.backend.order.OrderStatus.DELIVERED,
                    com.ut.edu.backend.order.OrderStatus.PAID
            );
            List<com.ut.edu.backend.order.Order> orders = orderRepository.findByCreatedAtAfterAndStatusIn(
                    startDate, revenueStatuses
            );

            // Calculate revenue metrics
            BigDecimal totalRevenue = orders.stream()
                    .map(com.ut.edu.backend.order.Order::getTotal)
            ...
            Map<String, BigDecimal> revenueByDate = orders.stream()
                    .collect(Collectors.groupingBy(
                            ...
                            Collectors.reducing(
                                    BigDecimal.ZERO,
                                    com.ut.edu.backend.order.Order::getTotal,
                                    BigDecimal::add
                            )
                    ));
    ...
    public Map<String, Object> getAllOrderClusters() {
        ...
        List<com.ut.edu.backend.order.Order> allOrders = orderRepository.findAll();
        ...
        for (com.ut.edu.backend.order.Order order : allOrders) {
