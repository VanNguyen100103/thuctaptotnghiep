...
public class CategoryController {
    ...
    /**
     * Load a category only if it belongs to the current store; cross-tenant
     * ids look like "not found" (anti-IDOR). Used by all write endpoints.
     */
    private Category findStoreCategory(Long id) {
    ...
    public ResponseEntity<?> getCategoryById(@PathVariable Long id) {
        try {
            Category category = findStoreCategory(id);
    ...
    public ResponseEntity<?> createCategory(
        ...
        try {
            ...
            // Create category - always belongs to the current store
            ...
            category.setStore(tenantGuard.currentStoreRef());

            // Set parent if provided (must belong to the same store)
            if (parentId != null) {
