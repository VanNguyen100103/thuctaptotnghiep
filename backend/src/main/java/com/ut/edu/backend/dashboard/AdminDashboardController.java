package com.ut.edu.backend.dashboard;
...
/**
 * Store Dashboard Controller (owner dashboard)
 * Statistics and analytics for store owners/managers.
 * Order/product queries are tenant-scoped by the Hibernate filter (the JWT
 * carries storeId -> TenantContext). User counts stay platform-wide because
 * customers are not tenant-scoped entities.
 */
@RestController
...
public class AdminDashboardController {
    ...
    public ResponseEntity<?> getRecentActivities(@RequestParam(defaultValue = "20") int limit) {
        try {
            ...
            // OPTIMIZED: Use database query with pagination instead of findAll()
            List<com.ut.edu.backend.user.User> recentUsers = userRepository.findRecentUsers(
                    org.springframework.data.domain.PageRequest.of(0, itemsPerType));
