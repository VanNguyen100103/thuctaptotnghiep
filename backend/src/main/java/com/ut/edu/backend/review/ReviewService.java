...
public class ReviewService {
    ...
    /**
     * Load a review only if its product belongs to the current store
     * (reviews are tenant-owned through their product); cross-tenant ids
     * look like "not found" (anti-IDOR). Used by moderation endpoints.
     */
    private Review findStoreReview(Long reviewId) {
        return reviewRepository.findById(reviewId)
                .filter(r -> r.getProduct() != null && tenantGuard.isCurrentStore(r.getProduct().getStore()))
    ...
    public Page<ReviewResponse> getAllReviews(Pageable pageable) {
        log.info("Store moderation: Getting all reviews of current store");

        // Review has no store_id column (tenant-owned through product), so the
        // Hibernate tenant filter does not apply here -> scope explicitly
        return reviewRepository.findByProductStoreId(tenantGuard.requireStore(), pageable)
                .map(ReviewResponse::fromEntity);
    }
    ...
    public ReviewResponse approveReview(Long reviewId) {
        log.info("Store moderation: Approving review {}", reviewId);

        Review review = findStoreReview(reviewId);
    ...
    public ReviewResponse rejectReview(Long reviewId) {
        log.info("Store moderation: Rejecting review {}", reviewId);
    ...
    public void adminDeleteReview(Long reviewId) {
        log.info("Store moderation: Deleting review {}", reviewId);
