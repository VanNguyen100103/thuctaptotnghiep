...
public record CreateGhnShipmentRequest(
        ...
        String note,
        /** Package dimensions in cm - optional, the POS "Bán giao hàng" panel sends these (defaults there: 10x10x10); null falls back to GhnShipmentService's own defaults for callers (like the "Tạo đơn test" modal) that don't expose them. */
        @Min(value = 1, message = "Length must be at least 1cm") Integer lengthCm,
        @Min(value = 1, message = "Width must be at least 1cm") Integer widthCm,
        @Min(value = 1, message = "Height must be at least 1cm") Integer heightCm) {
}
